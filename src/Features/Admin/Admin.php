...
class Admin implements HasPlugin {
    ...
    public function registerRoute() {
        ...
        Routes::map( 'invoize-quotation/:token', function ( $params ) use($nonce) {
            ...
            Routes::load( $this->plugin->getPluginDir() . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'front' . DIRECTORY_SEPARATOR . 'quotation-public-preview.php' );
        } );
        // OLD URL, redirect to new URL
        // domain.com/invoize-preview?token={some_token}
        Routes::map( 'invoize-preview', function () use($nonce) {
            invoizeValidateNonce( $nonce );
            $invToken = ( isset( $_GET['invoizeToken'] ) ? sanitize_text_field( wp_unslash( $_GET['invoizeToken'] ) ) : null );
        ...
        Routes::map( 'invoize-xendit-webhook', function () use($nonce) {
            $this->registerParam( $nonce );
            Routes::load( $this->plugin->getPluginDir() . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'front' . DIRECTORY_SEPARATOR . 'xendit-webhook-payment.php' );
        } );
    }
