...
class DatabaseUpdater
{
    ...
    public static function version02($installedDbVersion)
    {
        ...
        Recurring::chunk(100, function ($records) {
            foreach ($records as $record) {
                ...
                if ($invoiceMeta) {
                    foreach ($invoiceMeta as $key => $value) {
                        ...
                        // same with payment_status
                        if ($key === 'status') {
                            continue;
                        }

                        // already have the value in the metas, don't need this anymore
                        if ($key === 'reminders') {
                            continue;
                        }
                ...
                if ($recurringMeta) {
                    foreach ($recurringMeta as $key => $value) {
                        ...
                        if ($key === 'end') {
                            // recurring_end_in already exist.
                            // this 'end' is the default value, so create new meta for it
                            $key = 'recurring_end_in_default';
                        }
        ...
        Quotation::chunk(100, function ($records) {
            foreach ($records as $record) {
                $reminderMeta = invoize_mb_unserialize($record->getMeta('reminders'));
