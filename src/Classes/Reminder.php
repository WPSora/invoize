...
class Reminder
{
    ...
    public function run_reminder_action()
    {
        ...
        $this->_run_reminder_action(Quotation::class);
    }
    ...
    public function _run_reminder_action($model)
    {
        ...
        foreach ($activeReminders as $ar) {
            ...
            if (!empty($before) && !empty($before->meta_value) && !empty(unserialize($before->meta_value))) {
                $new = $this->check_reminder(unserialize($before->meta_value), $ar->ID, true, $model);
            ...
            if (!empty($after) && !empty($after->meta_value) && !empty(unserialize($after->meta_value))) {
                $new = $this->check_reminder(unserialize($after->meta_value), $ar->ID, false, $model);
    ...
    private function check_reminder(array $reminders, int $id, bool $isReminderBefore, $model = null): array
    {
