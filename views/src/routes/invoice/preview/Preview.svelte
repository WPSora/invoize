<script>
  ...
  export let params = {};
  /**
   * use this to get params for public preview. If non public, use above.
   * example:
   * http://localhost/wordpress/invoize-preview/?invoizeToken=bfc4a752df8b1cf0cb7d192c25ffb497
   * searchParams will get the bfc4a752df8b1cf0cb7d192c25ffb497 value by using searchParams.get("invoizeToken")
   */
  ...
  const getInvoiceDetail = async (callback) => {
    let api;
    if (isPublic) {
      api = `invoice/detail?invoizeToken=${searchParams.get("invoizeToken")}`;
    } else {
      page === "invoice"
        ? (api = `invoice/detail?invoizeToken=${params.invoizeToken}`)
        : page === "receipt"
          ? (api = `receipt/detail?invoizeToken=${params.invoizeToken}`)
          : null;
    }
    if (!api) {
  ...
  const setPublicLink = () => {
    token = isPublic ? searchParams.get("invoizeToken") : params?.invoizeToken;
    publicLink = `${invoize.base_url}/invoize-preview?invoizeToken=${token}`;
  };

  const checkIsPublic = () => {
    const path = location.pathname.split("/");
    const current = path.pop() || path.pop();
    if (current === "invoize-preview") {
      isPublic = true;
    }
    setPublicLink();
  };
