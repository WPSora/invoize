<script>
  ...
  import { banks, automaticPaypals, directPaypals, xenditKey } from "$lib/stores/settings-store";
...
{#if !isLoadingPayment && isEmptyCheck($banks) && isEmptyCheck($automaticPaypals) && isEmptyCheck($directPaypals) && !$xenditKey}
...
<div class="flex xl:flex-row xl:flex-nowrap flex-wrap justify-center mt-5">
  ...
  <TemplateOptions on:submit bind:isFinishModalOpen {isEdit} {isLoadingSettings} bind:isDiscountModalOpen bind:discountData bind:discountTemporaryEdit>
