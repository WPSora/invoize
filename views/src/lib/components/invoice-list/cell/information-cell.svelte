...
<div class="flex justify-between items-center w-full">
  <Button
    variant="link"
    class="p-0 text-xs flex justify-start w-10/12"
    on:click={() => (location.href = `#/invoice/${token}`)}>
    <div class="flex text-wrap text-start text-sm">
      {invoiceId}
      {customer}
    </div>
  </Button>
  <div class="flex flex-nowrap justify-center items-center gap-3 w-2/12">
    <!-- Email sent logo -->
    {#if isSent}
      <svg
    ...
    <!-- Woocommerce logo -->
    {#if isWoocommerce}
      <img alt="woocommerce" src="{invoize.plugin_url}public/wc-logo-gray.svg" class="w-6" />
    {/if}
  </div>
</div>
