<script>
  ...
  const saveTemplate = () => {
    ...
    for (let i = 0; i < selectedTemplate.cc.length; i++) {
      if(!isValidEmail(selectedTemplate.cc[i])){
        toast.error("Invalid email address, Please enter a valid email address for CC");
        return;
      }
    }
    ...
    for (let i = 0; i < selectedTemplate.bcc.length; i++) {
      if(!isValidEmail(selectedTemplate.bcc[i])){
        toast.error("Invalid email address, Please enter a valid email address for BCC");
        return;
      } 
    }
  ...
  const getTemplatesSettings = async () => {
    isLoading = true;
    try {
      const response = await createGetRequest("settings/retrieve?tab=templates");
      $emailTemplates = response.data.data.map((item) => {
        ...
        if(rawTemplate.cc?.length) {
          template.cc = rawTemplate.cc.includes(",") ? rawTemplate.cc.split(",") : [rawTemplate.cc];
        }

        if(rawTemplate.bcc?.length) {
          template.bcc = rawTemplate.bcc.includes(",") ? rawTemplate.bcc.split(",") : [rawTemplate.bcc];
        }
...
<Card class="p-4">
  <CardHeader class="space-y-0.5 md:p-6 px-0">
    <CardTitle class="md:text-lg text-base border-l-2 border-primary pl-2">E-Mail Notification</CardTitle>
      ...
      {:else}
        <div class="bg-secondary p-4 rounded-lg">
          <Table class="bg-white rounded-lg md:text-sm text-xs">
            <Caption class="md:text-sm text-xs">List of email templates used to send emails to users.</Caption>
            ...
            <Body>
              {#each $emailTemplates as emailTemplate, i (i)}
                <Row class="border-b-0">
                  <Cell>{i + 1}</Cell>
                  <Cell>{emailTemplate.name}</Cell>
                  <Cell class="text-center">
                    <EditButton on:click={() => toggleModal(emailTemplate)} />
                  </Cell>
                </Row>
              {/each}
            </Body>
...
<!-- Edit modal -->
<Dialog open={mailTemplateEditModalOpen} onOpenChange={() => (mailTemplateEditModalOpen = false)}>
    ...
    <div class="grid gap-4 py-4">
      <form on:submit|preventDefault={saveTemplate}>
        <div class="grid mb-3 w-full items-center gap-1.5">
          ...
          <Input type="text" id="name" disabled bind:value={selectedTemplate.name} />
        </div>
        <div class="grid mb-3 w-full items-center gap-1.5">
          <Label for="Subject">Subject</Label>
          <Input type="text" id="Subject" class="select-auto" required bind:value={selectedTemplate.subject} />
        </div>
        <div class="grid mb-3 w-full items-center gap-1.5">
          <Label for="Subject">CC</Label>
          <Tags name="{selectedTemplate.name}_cc" bind:value={selectedTemplate.cc}/>
        </div>
        <div class="grid mb-3 w-full items-center gap-1.5">
          <Label for="Subject">BCC</Label>
          <Tags name="{selectedTemplate.name}_bcc" bind:value={selectedTemplate.bcc}/>
        </div>
        <div class="grid mb-3 w-full items-center gap-1.5">
          <Label for="content">Content</Label>
          <Textarea id="content" rows={10} required bind:value={selectedTemplate.content} />
        </div>
      ...
      <div class="grid text-sm border-2 p-4 rounded-lg bg-slate-50">
        <div class="grid grid-cols-3 gap-1">
          ...
          <span class="text-slate-500">&#123;&#123; business_name &#125;&#125;</span>
          <span class="text-slate-700">Business name</span>
          <span></span>

          <span class="text-slate-500">&#123;&#123; client_name &#125;&#125;</span>
          <span class="text-slate-700">Client name</span>
          <span></span>

          <span class="text-slate-500">&#123;&#123; invoice_number &#125;&#125;</span>
          <span class="text-slate-700">Invoice Number</span>
          <span></span>

          <span class="text-slate-500">&#123;&#123; invoice_date &#125;&#125;</span>
