<script>
  ...
  const getDataList = async () => {
    try {
      ...
      if (clientIdParam) {
        const value = listStore.find((client) => client.id === parseInt(clientIdParam));
        if (value) {
          selectedBind = { label: value.name, value };
        }
      }
    } catch (err) {
      isLoadingData = false;
    ...
    {:else}
      ...
      {:else if isWcData}
        ...
        {:else if !isSearchingWcState && maxWcDataPage > currentWcDataPage && !isSearchingWcData}
          <Button class="text-xs flex justify-center w-full" variant="link" on:click={getMoreWcData}>Load more</Button>
        {/if}
      {:else if !isWcData}
