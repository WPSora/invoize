  ...
  <div class="flex flex-row gap-x-2">
    {#if !isDisabled}
      <Checkbox
        ...
        disabled={isDisabled || !$xenditKey}
        class={(isDisabled || !$xenditKey) && "border-muted-foreground cursor-not-allowed"} />
    {/if}
    <Label
      for="xendit-payment"
      class="flex flex-row flex-nowrap items-center gap-1 {(isDisabled || !$xenditKey) &&
