...
import StatusHeader from "$lib/components/recurring-list/header/status-header.svelte";
import StatusCell from "$lib/components/recurring-list/cell/status-cell.svelte";
...
export const setSearchTableColumns = (paginationStore, tableStore, updateTabList, params) => {
  ...
  const columns = table.createColumns([
    ...
    table.column({
      ...
      accessor: 'interval',
      cell: (item) => {
        const detail = item.row.original.detail;
        return createRender(ScheduleCell, { interval: item.value, detail })
      },
    }),
