...
export const savePaymentTabToSettingsStore = (res) => {
  ...
  settings?.activeAutomaticPaypalType && activeAutomaticPaypalType.set(settings.activeAutomaticPaypalType);
  settings?.xenditKey && xenditKey.set(settings.xenditKey);
  settings?.xenditPrimaryCurrency && xenditPrimaryCurrency.set(settings.xenditPrimaryCurrency);
...
export const saveInvoiceTabToSettingsStore = (res, isSaveToInvoiceStore = false, isEdit = false) => {
  const settings = {};
  res.map((item) => {
    settings[item.name] = item.value;
  });
  ...
  if (!isEdit) {
    ...
    settings?.termsAndConditions && termsAndConditions.set(settings.termsAndConditions);
  }
  settings?.reminders && reminders.set(settings.reminders);
  settings?.reminderGroups && reminderGroups.set(settings.reminderGroups);
  settings?.defaultReminderGroup && defaultReminderGroup.set(settings.defaultReminderGroup);
  settings?.downloadPaperSize && downloadPaperSize.set(settings.downloadPaperSize);
...
export const saveToInvoiceStore = (settings) => {
  ...
  settings?.termsAndConditions && termsAndConditionsInvoice.set(settings.termsAndConditions);
  settings?.defaultReminderGroup?.value?.after && selectedReminderAfter.set(settings.defaultReminderGroup.value.after);
  settings?.defaultReminderGroup?.value?.before && selectedReminderBefore.set(settings.defaultReminderGroup.value.before);
