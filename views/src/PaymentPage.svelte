<script>
  ...
  const proceedPayment = () => {
    if (payload.payment == PaymentMethod.BANK) {
    ...
    } else if (payload.payment === PaymentMethod.PAYPAL_DIRECT) {
      paymentDirectPaypalPage = true;
    } else if (payload.payment == PaymentMethod.XENDIT || payload.payment === PaymentMethod.PAYPAL_AUTO_CONFIRMATION) {
      ...
      toast.error(
        "There is something wrong with this payment. Please contact the merchant to change/update this invoice payment.",
      );
    }
  };
</script>
  ...
  <div class="max-w-xl mx-auto">
    ...
    <div class="text-center mb-4">
      <div class="flex justify-center items-center gap-3 mb-8">
        <img src="{invoize_payment?.business?.logo}" alt="Business Logo" class="h-7 w-auto" />
        <div class="text-xl">{invoize_payment?.business?.name}</div>
      </div>
      {#if invoize_payment.payment_status.toLowerCase() == "unpaid"}
        <h1 class="text-2xl font-bold text-gray-900 mb-1">One last step for finishing your order...</h1>
    ...
    <div class="p-6 mb-6 shadow-lg bg-white rounded-lg">
      ...
      {#if invoize_payment.invoice_status != "active"}
        <div class="justify-center flex flex-col items-center">
          ...
          <span class="text-xl"> Invoice is inactive </span>
      ...
      {#if invoize_payment.payment_status.toLowerCase() == "unpaid" && invoize_payment.invoice_status == "active"}
        {#if paymentBankPage || paymentDirectPaypalPage || paymentWoocommercePage}
          ...
          <div class="border border-gray-300 p-4 rounded-md mb-4">
            {#if paymentBankPage}
              <MultilineText text="{invoize_payment.bank_information}" class="text-gray-500" />
            {:else if paymentDirectPaypalPage}
              <a
                class="text-blue-500"
                href="{invoize_payment.paypal_information?.startsWith('http')
                  ? invoize_payment.paypal_information
                  : 'https://' + invoize_payment.paypal_information}"
                target="_blank">
                {invoize_payment.paypal_information}
              </a>
            ...
            {#each invoize_payment.highlight_columns as hcol}
              <div class="flex justify-between items-center pt-2 gap-12">
                <div class="flex flex-col">
                  ...
                  {#if hcol.description}
                    <small class="text-xs text-gray-500">{hcol.description}</small>
                  {/if}
                </div>

                <span class="text-xl font-bold text-primary">{hcol.value}</span>
              </div>
            {/each}
          </div>
        {/if}
    ...
    {:else if invoize_payment.payment_status.toLowerCase() == "unpaid"}
    ...
    <div class="mb-2 fixed bottom-0 left-0 w-full flex items-center justify-center gap-4">
      <span class="text-sm text-gray-500">
        Invoize by
        <a href="https://wpsora.com" target="_blank" class="text-primary"> WPSora </a>
      </span>
    </div>
  </div>
</div>
